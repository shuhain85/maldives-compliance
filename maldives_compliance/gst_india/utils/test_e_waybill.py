...
from maldives_compliance.gst_india.utils import load_doc, parse_datetime
...
from maldives_compliance.gst_india.utils.tests import (
    _append_taxes,
    append_item,
...
class TestEWaybill(FrappeTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.e_waybill_test_data = frappe._dict(
            frappe.get_file_json(
                frappe.get_app_path(
                    "maldives_compliance", "gst_india", "data", "test_e_waybill.json"
                )
            )
        )
    ...
    def test_get_all_item_details(self):
        ...
        hsn_codes = frappe.get_file_json(
            frappe.get_app_path(
                "maldives_compliance", "gst_india", "data", "hsn_codes.json"
            )
        )
