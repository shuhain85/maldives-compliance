...
async function set_gstin_options(frm) {
    const options = await maldives_compliance.get_gstin_options(frm.doc.company);
...
async function contains_gst_account(frm, taxes_table, account_field) {
    if (!frm.gst_accounts || frm.company !== frm.doc.company) {
        frm.gst_accounts = await maldives_compliance.get_account_options(frm.doc.company);
