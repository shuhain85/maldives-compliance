    ...
    frappe.ui.form.on(doctype, {
        after_save(frm) {
            ...
            frappe.confirm(message, function () {
                frappe.call({
                    method: "maldives_compliance.gst_india.overrides.party.update_docs_with_previous_gstin",
                    args: {
                        gstin: gstin || "",
                        gst_category,
                        docs_with_previous_gstin,
...
function validate_gstin(doctype) {
    frappe.ui.form.on(doctype, {
        gstin(frm) {
            ...
            gstin = maldives_compliance.validate_gstin(gstin);
            ...
            if (PAN_REGEX.test(pan)) {
                ...
                set_party_type(frm);
                if (doctype != "Address") {
                    maldives_compliance.set_pan_status(frm.get_field("pan"));
                }
            }
        },
    });
...
function validate_pan(doctype) {
    frappe.ui.form.on(doctype, {
        pan(frm) {
            ...
            pan = maldives_compliance.validate_pan(pan);
...
function set_gstin_options_and_status(doctype) {
    frappe.ui.form.on(doctype, {
        refresh(frm) {
            ...
            maldives_compliance.set_gstin_status(frm.get_field("gstin"), frm.doc);
        },
        gstin(frm) {
            maldives_compliance.set_gstin_status(frm.get_field("gstin"), frm.doc);
        },
...
async function set_gstin_options(frm) {
    ...
    field.set_data(await maldives_compliance.get_gstin_options(frm.doc.name, frm.doctype));
}
...
function set_gst_category(doctype) {
    frappe.ui.form.on(doctype, {
        gstin(frm) {
            frm.set_value(
                "gst_category",
                maldives_compliance.guess_gst_category(frm.doc.gstin, frm.doc.country)
            );
        },
    });
}
