{% include "maldives_compliance/gst_india/client_scripts/e_waybill_applicability.js" %}
...
function setup_e_waybill_actions(doctype) {
    ...
    frappe.ui.form.on(doctype, {
        ...
        setup(frm) {
            if (!maldives_compliance.is_api_enabled()) return;
        ...
        refresh(frm) {
            ...
            if (!maldives_compliance.is_api_enabled()) {
            ...
            if (frappe.perm.has_perm(frm.doctype, 0, "cancel", frm.doc.name)) {
                if (is_e_waybill_cancellable(frm)) {
                    maldives_compliance.add_divider_to_btn_group("e-Waybill");
                    ...
                    maldives_compliance.make_text_red("e-Waybill", "Cancel");
                }
                ...
                maldives_compliance.make_text_red("e-Waybill", "Mark as Cancelled");
            }
        },
        async on_submit(frm) {
            ...
            await frappe.xcall(
                "maldives_compliance.gst_india.utils.e_waybill.generate_e_waybill",
                { doctype: frm.doctype, docname: frm.doc.name }
            );
        },
        before_cancel(frm) {
            // if IRN is present, e-Waybill gets cancelled in e-Invoice action
            if (!maldives_compliance.is_api_enabled() || frm.doc.irn || !frm.doc.ewaybill)
                return;
...
function fetch_e_waybill_data(frm, args, callback) {
    ...
    frappe.call({
        method: "maldives_compliance.gst_india.utils.e_waybill.fetch_e_waybill_data",
...
function show_generate_e_waybill_dialog(frm) {
    const generate_action = values => {
        frappe.call({
            method: "maldives_compliance.gst_india.utils.e_waybill.generate_e_waybill",
            args: {
                doctype: frm.doctype,
    ...
    const json_action = async values => {
        const ewb_data = await frappe.xcall(
            "maldives_compliance.gst_india.utils.e_waybill.generate_e_waybill_json",
            {
                doctype: frm.doctype,
    ...
    const api_enabled = maldives_compliance.is_api_enabled();
...
function show_fetch_if_generated_dialog(frm) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "maldives_compliance.gst_india.utils.e_waybill.find_matching_e_waybill",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
...
function show_mark_e_waybill_as_generated_dialog(frm) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "maldives_compliance.gst_india.utils.e_waybill.mark_e_waybill_as_generated",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function show_cancel_e_waybill_dialog(frm, callback) {
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "maldives_compliance.gst_india.utils.e_waybill.cancel_e_waybill",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
    ...
    maldives_compliance.primary_to_danger_btn(d);
    d.show();
}
...
function show_mark_e_waybill_as_cancelled_dialog(frm) {
    ...
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "maldives_compliance.gst_india.utils.e_waybill.mark_e_waybill_as_cancelled",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
async function show_update_vehicle_info_dialog(frm) {
    ...
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "maldives_compliance.gst_india.utils.e_waybill.update_vehicle_info",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function show_update_transporter_dialog(frm) {
    ...
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "maldives_compliance.gst_india.utils.e_waybill.update_transporter",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
async function show_extend_validity_dialog(frm) {
    ...
    const d = new frappe.ui.Dialog({
        ...
        primary_action(values) {
            frappe.call({
                method: "maldives_compliance.gst_india.utils.e_waybill.extend_validity",
                args: {
                    doctype: frm.doctype,
                    docname: frm.doc.name,
                    values,
...
function schedule_e_waybill_extension(frm, dialog, scheduled_time) {
    ...
    if (values) {
        frappe.call({
            method: "maldives_compliance.gst_india.utils.e_waybill.schedule_ewaybill_for_extension",
            args: {
                doctype: frm.doctype,
                docname: frm.docname,
                values,
...
function validate_gst_transporter_id(dialog, doc) {
    maldives_compliance.validate_gst_transporter_id(
...
function get_primary_action_label_for_generation(doc) {
    const label = maldives_compliance.is_api_enabled()
        ? __("Generate")
        : __("Download JSON");
...
async function get_source_destination_address(frm, address_type) {
    let address = await frappe.call({
        method: "maldives_compliance.gst_india.utils.e_waybill.get_source_destination_address",
        args: {
            doctype: frm.doctype,
            docname: frm.docname,
