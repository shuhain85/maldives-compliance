...
frappe.ui.form.on("GST Settings", {
    setup(frm) {
        ...
        frm.set_query("gstin", "credentials", (_, cdt, cdn) => {
            ...
            return maldives_compliance.get_gstin_query(row.company);
        });
    },
...
function show_ic_api_promo(frm) {
    ...
    maldives_compliance.show_dismissable_alert(
        frm.layout.wrapper,
        alert_message,
        "primary",
        () => {
            frappe.xcall(
                "maldives_compliance.gst_india.doctype.gst_settings.gst_settings.disable_api_promo"
            );
        }
    );
}
    ...
    frm.add_custom_button(__("Update GST Category"), () => {
        frappe.msgprint({
            ...
            primary_action: {
                label: __("Update"),
                server_action:
                    "maldives_compliance.gst_india.doctype.gst_settings.gst_settings.enqueue_update_gst_category",
