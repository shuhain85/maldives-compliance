...
frappe.ui.form.on("GSTR 3B Report", {
    setup: function () {
        frappe.require("assets/maldives_compliance/js/gstr_2b.js");
    },
    ...
    onload: function (frm) {
        ...
        if (frm.doc.company)
            maldives_compliance.set_gstin_options(frm).then(options => {
    ...
    refresh: function (frm) {
        ...
        frm.add_custom_button(__("Download JSON"), function () {
            var w = window.open(
                frappe.urllib.get_full_url(
                    "/api/method/maldives_compliance.gst_india.doctype.gstr_3b_report.gstr_3b_report.make_json?" +
                        "name=" +
        ...
        frm.add_custom_button(__("View Form"), function () {
            frappe.call({
                method: "maldives_compliance.gst_india.doctype.gstr_3b_report.gstr_3b_report.view_report",
                args: {
                    name: frm.doc.name,
                },
                callback: function (r) {
        ...
        frm.add_custom_button(__("Regenerate 2B"), function () {
            ...
            gstr_2b.regenerate({
                ...
                return_period: maldives_compliance.get_period(
...
function set_options_for_year_month(frm) {
    const { options, current_year } = maldives_compliance.get_options_for_year("Monthly");
    frm.set_df_property("year", "options", options.slice(0, 3));

    if (!frm.is_new()) return;

    const last_month_name = maldives_compliance.last_month_name();
