...
frappe.ui.form.on(DOCTYPE, {
    async setup(frm) {
        ...
        new maldives_compliance.quick_info_popover(frm, tooltip_info);
    ...
    async company(frm) {
        ...
        const options = await maldives_compliance.set_gstin_options(frm, true);
...
class PurchaseReconciliationToolAction {
    ...
    export_data(selected_row) {
        ...
        const url =
            "maldives_compliance.gst_india.doctype.purchase_reconciliation_tool.purchase_reconciliation_tool.download_excel_report";
            ...
            },
            {
                ...
                get_query: async () => {
                    let { message: gstin_list } = await frappe.call({
                        method: "maldives_compliance.gst_india.utils.get_gstin_list",
...
async function download_gstr(
    ...
    if (company_gstin == "All")
        company_gstins = await maldives_compliance.get_gstin_options(frm.doc.company);
...
class EmailDialog {
    ...
    get_attachment() {
        ...
        frappe.call({
            method: "maldives_compliance.gst_india.doctype.purchase_reconciliation_tool.purchase_reconciliation_tool.generate_excel_attachment",
            args: {
    ...
    async get_recipients() {
        ...
        const { message } = await frappe.call({
            method: "maldives_compliance.gst_india.utils.get_party_contact_details",
            args: {
