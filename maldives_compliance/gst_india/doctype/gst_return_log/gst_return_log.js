...
frappe.ui.form.on("GST Return Log", {
        ...
        attachFields.forEach(field => {
            $(frm.fields_dict[field].wrapper).on(
                ...
                function (e) {
                    ...
                    const args = {
                        cmd: "maldives_compliance.gst_india.doctype.gst_return_log.gst_return_log.download_file",
                        file_field: field,
                        name: frm.doc.name,
    ...
    refresh(frm) {
        const [month_or_quarter, year] = maldives_compliance.get_month_year_from_period(
