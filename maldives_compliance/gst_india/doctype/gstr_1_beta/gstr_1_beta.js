...
frappe.provide("maldives_compliance");
...
frappe.ui.form.on(DOCTYPE, {
    async setup(frm) {
        ...
        frappe.realtime.on("is_not_latest_gstr1_data", message => {
            ...
            const [month_or_quarter, year] =
                maldives_compliance.get_month_year_from_period(filters.period);
    ...
    async company(frm) {
        ...
        const options = await maldives_compliance.set_gstin_options(frm);
...
class GSTR1 {
    ...
    render_view_groups() {
        ...
        this.viewgroup = new maldives_compliance.ViewGroup({
            $wrapper: wrapper,
    ...
    setup_filter_button() {
        this.filter_group = new maldives_compliance.FilterGroup({
            doctype: DOCTYPE,
    ...
    async show_suggested_jv_dialog() {
        ...
        const { message: je_details } = await frappe.call({
            method: "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_beta.get_journal_entries",
    ...
    create_journal_entry_dialog(je_details) {
        const dialog = new frappe.ui.Dialog({
            ...
            primary_action: values => {
                ...
                frappe.call({
                    method: "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_beta.make_journal_entry",
                    args: {
                        company,
                        company_gstin,
                        month_or_quarter,
...
class TabManager {
    ...
    filter_data(data, filters) {
        ...
        return data.filter(row => {
            return filters.every(filter =>
                maldives_compliance.FILTER_OPERATORS[filter[2]](
                    filter[3] || "",
                    row[filter[1]] || ""
                )
            );
    ...
    setup_datatable(wrapper, data, columns) {
        ...
        this.datatable = new maldives_compliance.DataTableManager({
...
class BooksTab extends GSTR1_TabManager {
    ...
    download_books_as_excel() {
        const url =
            "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_export.download_books_as_excel";
...
class FiledTab extends GSTR1_TabManager {
    ...
    download_filed_as_excel() {
        const url =
            "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_export.download_filed_as_excel";
    ...
    download_filed_json() {
        ...
        function get_json_data(dialog) {
            ...
            frappe.call({
                method: "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_export.get_gstr_1_json",
                args: {
                ...
                callback: r => {
                    maldives_compliance.trigger_file_download(
...
class ReconcileTab extends FiledTab {
    ...
    download_reconcile_as_excel() {
        const url =
            "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_export.download_reconcile_as_excel";
...
class FileGSTR1Dialog {
    ...
    async file_gstr1_data() {
        ...
        this.filing_dialog = new frappe.ui.Dialog({
            ...
            primary_action: async () => {
                ...
                maldives_compliance.validate_pan(pan);

                // generate otp
                await maldives_compliance.generate_evc_otp(
        ...
        taxpayer_api.call({
            method: "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_beta.perform_gstr1_action",
            args: {
    ...
    update_actions_for_filing(pan) {
        ...
        this.filing_dialog.set_secondary_action(() => {
            maldives_compliance.generate_evc_otp(this.frm.doc.company_gstin, pan, "R1");
        });
    }
...
class GSTR1Action extends FileGSTR1Dialog {
    ...
    async mark_as_unfiled() {
        ...
        frappe.call({
            method: "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_beta.mark_as_unfiled",
        ...
        setTimeout(
            async () => {
                const { message } = await taxpayer_api.call({
                    method: `maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_beta.check_action_status`,
...
function is_gstr1_api_enabled() {
    return (
        maldives_compliance.is_api_enabled() &&
...
async function set_default_company_gstin(frm) {
    ...
    const { message: gstin_list } = await frappe.call(
        "maldives_compliance.gst_india.utils.get_gstin_list",
        { party: company }
    );
...
function update_filing_preference(frm) {
    ...
    frappe.call({
        method: "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_beta.get_filing_preference_from_log",
...
function set_options_for_month_or_quarter(frm) {
    ...
    if (frm.doc.year === current_year) {
        ...
        options = maldives_compliance.MONTH.slice(0, current_month_idx + 1);
    } else if (frm.doc.year === "2017") {
        // Options for 2017 from July to December
        options = maldives_compliance.MONTH.slice(6);
    } else {
        options = maldives_compliance.MONTH;
    }
...
async function get_net_gst_liability(frm) {
    ...
    const response = await frappe.call({
        method: "maldives_compliance.gst_india.doctype.gstr_1_beta.gstr_1_beta.get_net_gst_liability",
        args: {
            company,
            company_gstin,
            month_or_quarter,
...
function refresh_filing_preference(frm) {
    ...
    frm.$wrapper.find(".update-filing-preference").click(async function (e) {
        ...
        const month = maldives_compliance.MONTH.indexOf(month_or_quarter) + 1;
        const period = `${String(month).padStart(2, "0")}${year}`;

        const { message: new_preference } = await taxpayer_api.call({
            method: "maldives_compliance.gst_india.utils.gstin_info.get_and_update_filing_preference",
...
function set_options_for_year(frm) {
    const { options, current_year } = maldives_compliance.get_options_for_year(
