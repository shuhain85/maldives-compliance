...
from maldives_compliance.install import POST_INSTALL_PATCHES
...
class TestPatches(FrappeTestCase):
    def test_post_install_patch_exists(self):
        for patch in POST_INSTALL_PATCHES:
            self.assertTrue(
                frappe.get_attr(
                    f"maldives_compliance.patches.post_install.{patch}.execute"
                )
            )

    def test_patches_exists(self):
        patches = get_patches_from_app("maldives_compliance")
