...
async function update_gst_details(frm, event) {
    ...
    const party_type = maldives_compliance.get_party_type(frm.doc.doctype).toLowerCase();
    ...
    maldives_compliance.fetch_and_update_gst_details(frm, args);
}

maldives_compliance.fetch_and_update_gst_details = function (frm, args, method) {
    frappe.call({
        method:
            method ||
            "maldives_compliance.gst_india.overrides.transaction.get_gst_details",
        args,
        async callback(r) {
            if (!r.message) return;
...
async function _set_gstin_status(frm, gstin_field_name) {
    ...
    if (!gstin_doc) {
        gstin_doc = await maldives_compliance.set_gstin_status(gstin_field, frm.doc);
        ...
        supplier(frm) {
            ...
            setTimeout(() => {
                ...
                erpnext.utils.get_party_details(
                    frm,
                    "maldives_compliance.gst_india.overrides.transaction.get_party_details_for_subcontracting",
                    args,
                    () => {
