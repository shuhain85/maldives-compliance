...
Object.assign(maldives_compliance, {
    MONTH: [
        "January",
        "February",
        "March",
    ...
    get_gstin_query(party, party_type = "Company") {
        ...
        return {
            query: "maldives_compliance.gst_india.utils.get_gstin_list",
            params: { party, party_type },
        };
    },

    async get_gstin_options(party, party_type = "Company") {
        const { query, params } = maldives_compliance.get_gstin_query(party, party_type);
    ...
    async get_account_options(company) {
        if (!company) return;
        const { message } = await frappe.call({
            method: "maldives_compliance.gst_india.utils.get_all_gst_accounts",
            args: {
                company,
            },
        });
    ...
    async set_gstin_status(field, doc, force_update) {
        ...
        let { message } = await frappe.call({
            method: "maldives_compliance.gst_india.doctype.gstin.gstin.get_gstin_status",
        ...
        field.set_description(
            maldives_compliance.get_gstin_status_desc(
                message?.status,
    ...
    async set_pan_status(field, force_update = null) {
        ...
        let { message } = await frappe.call({
            method: "maldives_compliance.gst_india.doctype.pan.pan.get_pan_status",
        ...
        pan_desc.find(".refresh-pan").on("click", async function () {
            await maldives_compliance.set_pan_status(field, true);
        });
    ...
    validate_gst_transporter_id(transporter_id, doc) {
        ...
        frappe.call({
            method: "maldives_compliance.gst_india.doctype.gstin.gstin.validate_gst_transporter_id",
    ...
    set_gstin_refresh_btn(field, doc) {
        ...
        refresh_btn.on("click", async function () {
            await maldives_compliance.set_gstin_status(field, doc, true);
        });
    },
    ...
    is_api_enabled(settings) {
        ...
        return settings.enable_api && maldives_compliance.can_enable_api(settings);
    },

    is_e_invoice_enabled() {
        return maldives_compliance.is_api_enabled() && gst_settings.enable_e_invoice;
    },
...
// DO NOT USE IN CODE
window.ic = window.maldives_compliance;
